...
if ($mode === 'history') {
    $historyFilter = $accessCondition;
    if ($find_query) {
        $historyFilter .= " AND calls.sitedomen = '$find_query'";
    }
    if ($phone_query) {
        $historyFilter .= " AND calls.phonenumber = '$phone_query'";
    }
    ...
    $sql_pages = "SELECT COUNT(*) FROM calls WHERE $historyFilter";
    $total_pages = findPagesNumber($sql_pages);

    $summaryTotalResult = pg_query($conn, "SELECT COUNT(*) FROM calls WHERE $historyFilter");
    $summaryTodayResult = pg_query($conn, "SELECT COUNT(*) FROM calls WHERE $historyFilter AND calls.nextcalldate = '$today'");
